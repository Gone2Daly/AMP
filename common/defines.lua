...
country = {
	...
	CAMPAIGN_EVENT_BASE_TIME = 7, -- once every 6 weeks.
	CAMPAIGN_EVENT_MIN_TIME = 7 ,-- never more often than 3 weeks between
	CAMPAIGN_EVENT_STATE_SCALE = -3,	-- every non-colonial state reduces by 3 days.
	CAMPAIGN_DURATION = 1,	-- a campaign lasts these amount of months
	...
	AI_SUPPORT_REFORM = 0.001, -- At least this many % needs to support a reform for the AI to take it
	BASE_MONTHLY_DIPLOPOINTS = 0.9, -- Base value gain for diplomatic actions each month
	DIPLOMAT_TRAVEL_TIME = 3,
	PROVINCE_OVERSEAS_PENALTY = 0.005, -- Each province req. this many goods flagged as overseas penalty 
	NONCORE_TAX_PENALTY = -0.3, -- -5% for each non-core in state
	BASE_TARIFF_EFFICIENCY = 0.2, -- baseline tariff efficiency
	COLONY_FORMED_PRESTIGE = 0.2, -- prestige from founding a colony.
	CREATED_CB_VALID_TIME = 18,  -- how many months 
	LOYALTY_BOOST_ON_PARTY_WIN = 0.2,
	...
	MOBILIZATION_SPEED_RAILS_MULT = 2.0, -- Speed modifier for raising troops. It's max value for max railways level in state.
...
economy = {
	...
	MIN_NUM_FACTORIES_PER_STATE_BEFORE_DELETING_LASSIEZ_FAIRE = 5, -- Min number of factories per state before starting to delete under Lassiez Faire
	...
	FACTORY_UPGRADE_EMPLOYEE_FACTOR = 0.90, -- determines how close to the employee limit we need to be before "upgrade all" will upgrade/expand a given factory (1 = 100%).
	...
	TRADE_CAP_LOW_LIMIT_NAVAL = 0.05, 			-- the lowest % the slider can go for naval units
...
military = {
	...
	SUPPLY_RANGE = 250,
	POP_MIN_SIZE_FOR_REGIMENT_PROTECTORATE_MULTIPLIER = 9,
	...
	COMBATLOSS_WAR_EXHAUSTION = 7, -- base war exhastion in combat
	LEADER_MAX_RANDOM_PRESTIGE = 0, -- max percent of prestige, when randomizing stats for leaders.
	LEADER_AGE_DEATH_FACTOR = 6, -- higher value means leaders live longer
	...
	AI_BIGSHIP_PROPORTION = 0.6, -- fraction of ships in the navy that should be ships of the line
	...
	AI_ARMY_TAXBASE_FRACTION = 0.50, -- max of tax base that AI will spend on army supply costs (based on peacetime costs)
	AI_NAVY_TAXBASE_FRACTION = 0.50, -- max of tax base that AI will spend on navy supply costs (based on peacetime costs)
	...
	SIEGE_ATTRITION = 3, -- fixed attrition on sieging units
...
diplomacy = {
	...
	WAR_FAILED_GOAL_MILITANCY = 0.2,
	WAR_FAILED_GOAL_PRESTIGE_BASE = -5,
	WAR_FAILED_GOAL_PRESTIGE = -0.05,
	...
	DISCREDIT_INFLUENCE_COST = 20,

	EXPELADVISORS_RELATION_ON_ACCEPT = -5,
	EXPELADVISORS_INFLUENCE_COST = 40,
	...
	INVESTMENT_INFLUENCE_DEFENSE = 1,	-- maximum defense factor in sphere of having invested in a country
	...
	TENSION_ON_CB_DISCOVERED = 1,
	TENSION_ON_REVOLT = 1,
	TENSION_WHILE_CRISIS = -100,
	CRISIS_COOLDOWN_MONTHS = 6,
	CRISIS_BASE_CHANCE = -100,
	...
	TWS_GRACE_PERIOD_DAYS = 360, -- Ticking War Score delay before it starts falling down for not fulfilling CB.
	TWS_CB_LIMIT_DEFAULT = 100,
	TWS_FULFILLED_IDLE_SPACE = 0.5, -- How much % the CB fulfillment must done, so TWS starts ticking.
...
pops = {
	...
	LARGE_POPULATION_LIMIT = 10000000000,
...
ai =
{
	...
	CHANCE_BUILD_NAVAL_BASE = 0.1,
	CHANCE_BUILD_FORT = 0.01,
	CHANCE_INVEST_POP_PROJ = 0.3,
	...
	TWS_AWARENESS_SCORE_ASPECT = 0.9, -- AI will not add any more CBs when TWS is more then X% of total WS. (to not destroy the progress)
	PEACE_BASE_RELUCTANCE = 40, -- AI base stubbornness to refuse peace (always applied)
	...
	PEACE_WAR_DIRECTION_WINNING_MULT = 10.0, -- Multiplies AI emphasis on war direction if it's the one making gains
	...
	AGGRESSION_UNCIV_BONUS = 50, -- additional AI civ aggression against uncivs
	...
	MONTHS_BEFORE_DISBAND = 4, -- Months from start date before AI will disband armies/navies (to avoid disbands because of early economic turmoil)
},
